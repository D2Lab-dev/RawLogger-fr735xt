...
class RL_App extends App.AppBase {
  ...
  function initActivity() {
    if($.RL_oActivitySession == null) {
      ...
      if($.RL_oSettings.bSensorAcceleration_HD and iFitFields >= 3 and iFitBytes >= 6*SAMPLERATE_ACCELERATION_HD) {
        $.RL_oFitField_SensorAccelerationX_HD = $.RL_oActivitySession.createField("SensorAccelerationX_HD", RL_App.FITFIELD_SENSORACCELERATIONX_HD, FitContributor.DATA_TYPE_SINT16, { :count => SAMPLERATE_ACCELERATION_HD, :mesgType => FitContributor.MESG_TYPE_RECORD, :units => Ui.loadResource(Rez.Strings.unitSensorAcceleration) });
        $.RL_oFitField_SensorAccelerationY_HD = $.RL_oActivitySession.createField("SensorAccelerationY_HD", RL_App.FITFIELD_SENSORACCELERATIONY_HD, FitContributor.DATA_TYPE_SINT16, { :count => SAMPLERATE_ACCELERATION_HD, :mesgType => FitContributor.MESG_TYPE_RECORD, :units => Ui.loadResource(Rez.Strings.unitSensorAcceleration) });
        $.RL_oFitField_SensorAccelerationZ_HD = $.RL_oActivitySession.createField("SensorAccelerationZ_HD", RL_App.FITFIELD_SENSORACCELERATIONZ_HD, FitContributor.DATA_TYPE_SINT16, { :count => SAMPLERATE_ACCELERATION_HD, :mesgType => FitContributor.MESG_TYPE_RECORD, :units => Ui.loadResource(Rez.Strings.unitSensorAcceleration) });
        iFitFields -= 3;
        iFitBytes -= 6*SAMPLERATE_ACCELERATION_HD;
      ...
      if(bHighDefListener_Acceleration) {
		...
		// Sys.println("Init Accelerometer listener.");
		var options = {
		    :period => 1,               				// 1 second sample time
	        :sampleRate => SAMPLERATE_ACCELERATION_HD,	// sample rate
	        :enableAccelerometer => true				// enable accelerometer
		};
        Sensor.registerSensorDataListener(method(:onSensorData), options);
        //Sensor.registerSensorDataListener(method(:onSensorData), { :period => 1, :accelerometer =>  { :enabled => true, :sampleRate => SAMPLERATE_ACCELERATION_HD } });
      }
    }
  }
  ...
  function onSensorData(_oData) {
    ...
    if($.RL_oData.aiSensorAccelerationX_HD != null and $.RL_oFitField_SensorAccelerationX_HD != null) {
      $.RL_oFitField_SensorAccelerationX_HD.setData($.RL_oData.aiSensorAccelerationX_HD.slice(0, SAMPLERATE_ACCELERATION_HD));
      //Sys.println(SAMPLERATE_ACCELERATION_HD);
      //Sys.println($.RL_oData.aiSensorAccelerationX_HD);
    }
    if($.RL_oData.aiSensorAccelerationY_HD != null and $.RL_oFitField_SensorAccelerationY_HD != null) {
      $.RL_oFitField_SensorAccelerationY_HD.setData($.RL_oData.aiSensorAccelerationY_HD.slice(0, SAMPLERATE_ACCELERATION_HD));
    }
    if($.RL_oData.aiSensorAccelerationZ_HD != null and $.RL_oFitField_SensorAccelerationZ_HD != null) {
      $.RL_oFitField_SensorAccelerationZ_HD.setData($.RL_oData.aiSensorAccelerationZ_HD.slice(0, SAMPLERATE_ACCELERATION_HD));
    }
